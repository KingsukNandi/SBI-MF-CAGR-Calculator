...
describe("external store", () => {
  ...
  it("keeps every subscriber of a shared id in step", async () => {
    ...
    expect(seen[0].keys).toEqual(["cagr", "date"]);
  ...
  it("resetOrder clears storage and notifies", async () => {
    ...
    expect(getOrderSnapshot("t4").keys).toBeNull();
    expect(load("t4")).toBeNull();
  });
});

describe("widths", () => {
  it("clamps to a usable range", async () => {
    const { clampWidth, MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH } =
      await import("./columnOrder.js");
    expect(clampWidth(5)).toBe(MIN_COLUMN_WIDTH);
    expect(clampWidth(99999)).toBe(MAX_COLUMN_WIDTH);
    expect(clampWidth(140.6)).toBe(141);
  });

  it("round-trips a width map", async () => {
    withWindow(makeStorage());
    const { saveWidths, loadWidths } = await import("./columnOrder.js");
    saveWidths("holdings", { scheme: 240, xirr: 90 });
    expect(loadWidths("holdings")).toEqual({ scheme: 240, xirr: 90 });
  });

  it("clamps stored widths on the way back in", async () => {
    withWindow(makeStorage({ "mf:colw:holdings": '{"scheme":10,"xirr":99999}' }));
    const { loadWidths, MIN_COLUMN_WIDTH, MAX_COLUMN_WIDTH } =
      await import("./columnOrder.js");
    expect(loadWidths("holdings")).toEqual({
      scheme: MIN_COLUMN_WIDTH,
      xirr: MAX_COLUMN_WIDTH,
    });
  });

  it("ignores non-numeric entries and corrupt JSON", async () => {
    withWindow(makeStorage({ "mf:colw:a": '{"x":"wide","y":180}' }));
    const { loadWidths } = await import("./columnOrder.js");
    expect(loadWidths("a")).toEqual({ y: 180 });

    withWindow(makeStorage({ "mf:colw:b": "{not json" }));
    expect((await import("./columnOrder.js")).loadWidths("b")).toBeNull();
  });

  it("setWidth updates the snapshot without disturbing the order", async () => {
    withWindow(makeStorage());
    const { setOrder, setWidth, getOrderSnapshot, __resetStoreForTests } =
      await import("./columnOrder.js");
    __resetStoreForTests();
    setOrder("t5", cols("b", "a"));
    setWidth("t5", "a", 200);
    const snap = getOrderSnapshot("t5");
    expect(snap.keys).toEqual(["b", "a"]);
    expect(snap.widths).toEqual({ a: 200 });
  });
});

describe("visibility", () => {
  // Stored as the HIDDEN set, so a newly shipped column defaults to visible.
  it("round-trips hidden keys", async () => {
    withWindow(makeStorage());
    const { saveHidden, loadHidden } = await import("./columnOrder.js");
    saveHidden("holdings", new Set(["units", "averageNAV"]));
    expect(loadHidden("holdings")).toEqual(["units", "averageNAV"]);
  });

  it("ignores non-strings and corrupt JSON", async () => {
    withWindow(makeStorage({ "mf:colh:a": '["units",7,null]' }));
    const { loadHidden } = await import("./columnOrder.js");
    expect(loadHidden("a")).toEqual(["units"]);

    withWindow(makeStorage({ "mf:colh:b": "nope" }));
    expect((await import("./columnOrder.js")).loadHidden("b")).toBeNull();
  });

  it("setHidden deduplicates and notifies", async () => {
    withWindow(makeStorage());
    const { setHidden, getOrderSnapshot, subscribeToOrder, __resetStoreForTests } =
      await import("./columnOrder.js");
    __resetStoreForTests();
    let notified = 0;
    subscribeToOrder("t6")(() => { notified += 1; });
    setHidden("t6", ["units", "units", "xirr"]);
    expect(notified).toBe(1);
    expect(getOrderSnapshot("t6").hidden).toEqual(["units", "xirr"]);
  });

  // One control, one predictable outcome.
  it("resetOrder clears order, widths and visibility together", async () => {
    withWindow(makeStorage());
    const { setOrder, setWidth, setHidden, resetOrder, getOrderSnapshot, __resetStoreForTests } =
      await import("./columnOrder.js");
    __resetStoreForTests();
    setOrder("t7", cols("b", "a"));
    setWidth("t7", "a", 200);
    setHidden("t7", ["b"]);
    resetOrder("t7");
    const snap = getOrderSnapshot("t7");
    expect(snap.keys).toBeNull();
    expect(snap.widths).toEqual({});
    expect(snap.hidden).toEqual([]);
  });

  it("no-ops during SSR", async () => {
    delete globalThis.window;
    const { loadWidths, loadHidden, saveWidths, saveHidden } =
      await import("./columnOrder.js");
    expect(loadWidths("x")).toBeNull();
    expect(loadHidden("x")).toBeNull();
    expect(saveWidths("x", { a: 1 })).toBe(false);
    expect(saveHidden("x", ["a"])).toBe(false);
  });
});
